...
- ✅ **Game-specific templates** - Which asset templates to extract (Anno 1800: 993, Anno 117: 945)
- ✅ **Priority fixlists** - Which templates get expensive dependency resolution (Anno 1800: 988, Anno 117: 919)
...
**Content**: 993 template entries for Anno 1800
...
- If file missing: uses hardcoded `templates_used[]` array as fallback (much shorter — the external file is the authoritative list)
...
**Content**: 945 template entries for Anno 117

**Processing Phase**: Phase 3 (Asset Extraction)

**Usage**:
- Loaded when Anno 117 game detected
- Similar in size to Anno 1800 (945 vs 993)
...
**Content**: 988 entries for Anno 1800
...
- Processes properties of templates in fixlist (significant performance cost)
...
**Content**: 919 entries for Anno 117

**Usage**:
- Applied only when Anno 117 game detected
- Similar in size to Anno 1800 (919 vs 988)
...
public static string[] templates_used = new string[] { 
    "Skin", "Slot", "Street", ... // short hardcoded fallback
};

// If 02_Processing_Rules/Anno1800_Fixlist.txt missing:
public static string[] fixlist = new string[] { 
    "ActionPauseBuilding", "AirShip", ... // short hardcoded fallback
};
...
| `-g:GUID` | Extract a single asset by GUID only |
...
| `--create-asset-mods` | Create one Mod Loader folder per generated asset XML |
| `--source-extraction-only` | Phase 1 only — extract source XML from RDA, then stop |
| `--update-templates` | Update the template list config from the game's `templates.xml` |
| `--compare-templates` | Diff config templates against current game templates |
| `--auto-templates` | Dynamically extract all templates from the game at runtime |
...
AssetProcessor "C:\Anno1800" "./output" english -c -f -t

# Automatically loads:
# - 01_Templates/Anno1800_Templates.txt (993 templates)
# - 02_Processing_Rules/Anno1800_Fixlist.txt (988 templates)
...
AssetProcessor "C:\Anno1800" "./output" english -c -f -t -u:config/subset_templates.txt
...
AssetProcessor "C:\Anno1800" "./output" english -c -f -t -x:config/priority_fixlist.txt
...
AssetProcessor "C:\Anno1800" "./output" english -c -f -t -d
...
|----------|-----------|----------|
| Extraction Templates | 993 | 945 |
| Fixlist Templates | 988 | 919 |
| Comment Whitelist Entries | 814 | 1,670 |

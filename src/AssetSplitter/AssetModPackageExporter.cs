...
public static partial class AssetModPackageExporter
{
    ...
    public static AssetModPackageExportResult Export(PipelineContext context, string gameType)
    {
        ...
        {
            if (!TryCreatePackage(context, gameType, sourceRoot, outputRoot, file, usedDirectories, indexes))
            {
                if (context.DebugMode)
        ...
        if (!context.IsSingleAssetMode)
        {
            WriteTemplateIndexes(outputRoot, indexes);
            ModReadmeWriter.WriteExportSummary(outputRoot, gameType, created, skipped, singleAssetMode: false);
        }
        else if (created > 0)
        {
            ModReadmeWriter.WriteExportSummary(outputRoot, gameType, created, skipped, singleAssetMode: true);
        }
        if (context.DebugMode)
        ...
        Dictionary<string, List<AssetModIndexEntry>> indexes)
    {
        ...
        if (parts.Length < 2 && !context.IsSingleAssetMode)
            return false;
        ...
        string templateName = parts.Length >= 2 ? parts[0] : ReadTemplateName(file);
        if (string.IsNullOrWhiteSpace(templateName))
            templateName = "Asset";

        string templateFolder = AssetTextSanitizer.SanitizeFileNamePart(templateName, MaxFolderNameLength);
        ...
        {
            string preferredModRoot = Path.Combine(outputRoot, templateFolder, modFolderName);
        ...
        File.Copy(file, assetsPath, true);
        ...
        if (!context.IsSingleAssetMode)
            AddIndexEntry(indexes, templateFolder, metadata, Path.GetFileName(modRoot));
    ...
    private static void WriteTemplateIndexes(string outputRoot, Dictionary<string, List<AssetModIndexEntry>> indexes)
    {
        foreach (var (templateFolder, entries) in indexes)
        {
            string indexPath = Path.Combine(outputRoot, templateFolder, "INDEX.md");
            IReadOnlyList<string> tableHeader = ModReadmeWriter.GetIndexTableHeader();
            List<string> lines =
            [
                ModReadmeWriter.GetIndexTitle(templateFolder),
                "",
                ModReadmeWriter.GetIndexBrowseLine(),
                "",

...
internal static class AssetProcessorFileSystem
{
    ...
    /// <summary>
    /// XML paths for GUID indexing after extraction: BaseAssetGUID staging first, then output root.
    /// Staging is listed first so inherited assets win when the same GUID appears in both places.
    /// </summary>
    public static IEnumerable<string> CollectGuidIndexFilePaths(string assetOut)
    {
        string stagingDir = Path.Combine(assetOut, "BaseAssetGUID");
    ...
    public static string? FindAssetFile(string assetOutputDirectory, string guid, bool searchTemplateFolders)
    {
        ...
        string stagingDirectory = Path.Combine(assetOutputDirectory, "BaseAssetGUID");

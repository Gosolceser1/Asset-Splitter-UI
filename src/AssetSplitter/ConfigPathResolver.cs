...
public static class ConfigPathResolver
{
    public static string Resolve(string basePath, string subfolder, string filename)
    {
        ...
        {
            string direct = Path.Combine(basePath, subfolder, filename);
            if (File.Exists(direct))
            {
                return direct;
            }
        }
        string outputConfig = Path.Combine(AppContext.BaseDirectory, "config", subfolder, filename);
        ...
        {
            string resolved = Path.GetFullPath(dev);

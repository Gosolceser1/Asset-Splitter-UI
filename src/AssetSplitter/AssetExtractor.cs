...
public static class AssetExtractor
{
        ...
        string gameType)
    {
        ...
        while (pending.TryDequeue(out string? currentGuid) && currentGuid is not null)
        {
            ...
            string outPath = GetExtractOutputPath(context, filename, !string.IsNullOrEmpty(baseAssetGuid));

            AssetDocumentSaver.SaveExtractedAsset(context, xmlDocument, outPath, !context.AssetModOpsWrap);
        ...
        Parallel.ForEach(
            ...
            assetNode =>
            {
                ...
                try
                {
                    ...
                    string outputPath = GetExtractOutputPath(context, fileName, hasBase).Replace("\n", "").Replace("\r", "");

                    AssetDocumentSaver.SaveExtractedAsset(context, localDoc, outputPath, !context.AssetModOpsWrap);
                    ...
                    if (context.DebugMode)
                    {
                        context.Log.Debug(string.Format(
                            ...
                            templateName,
                            outputPath,
                            hasBase && context.AssetFix ? "BaseAssetGUID" : "main"));
                    }
    ...
    /// <summary>Staging folder is only used when <see cref="PipelineContext.AssetFix"/> (-f) will merge parents in Phase 6.</summary>
    private static string GetExtractOutputPath(PipelineContext context, string fileName, bool hasBaseAssetGuid)
    {
        if (hasBaseAssetGuid && context.AssetFix)
        {
            return Path.Combine(OutputDirectoryManager.EnsureBaseAssetGuidStagingDirectory(context), fileName);
        }

        return Path.Combine(context.AssetOut, fileName);
    }
}

...

{
...

    {
        ...
        InsertBlockBefore(lines, t("readmeGuideQuickStartHeader"), GetBuildFootprintLines(context, t, includeGeneratedAt: true));
...

    {
        ...
        InsertBlockBefore(lines, t("readmeFileToEditHeader"), GetBuildFootprintLines(context, t, includeGeneratedAt: true));
...

    {
...

        {
            ...
            t("readmeSummaryTitle"),

            "",

            string.Format(t("readmeSummaryGame"), gameDisplayName),

        };

        lines.AddRange(GetBuildFootprintLines(context, t, includeGeneratedAt: true));

        lines.AddRange(
        [
            string.Format(t("readmeSummaryCreated"), created.ToString("N0")),

            string.Format(t("readmeSummarySkipped"), skipped.ToString("N0")),

            "",

            t("readmeSummaryIntro"),

            "",

            t("readmeSummaryGuide"),
...

        {
            ...
            lines.Add(t("readmeSummaryIndex"));

            lines.Add("");

            lines.Add(t("readmeSummaryWarning"));
...

        {
            ...
            lines.Add(t("readmeSummarySingle"));
    ...
    public static string GetIndexTitle(string templateFolder, string readmeLanguage) =>

        string.Format(ConsoleMessages.GetForLanguage("readmeIndexTitle", readmeLanguage), templateFolder);



    public static string GetIndexBrowseLine(string readmeLanguage) =>

        string.Format(ConsoleMessages.GetForLanguage("readmeIndexBrowseLine", readmeLanguage), ModdingGuideFileName);



    public static IReadOnlyList<string> GetIndexTableHeader(string readmeLanguage) =>
    ...
    private static IReadOnlyList<string> GetBuildFootprintLines(PipelineContext context, Func<string, string> t, bool includeGeneratedAt)
    {
        var lines = new List<string>();
        string gameBuild = GeneratedXmlFootprint.BuildGameBuildDescription(context);
        if (!string.IsNullOrWhiteSpace(gameBuild))
        {
            lines.Add(string.Format(t("readmeGameBuildLabel"), gameBuild));
        }

        string buildTime = GeneratedXmlFootprint.BuildGameBuildTimeDescription(context);
        if (!string.IsNullOrWhiteSpace(buildTime))
        {
            lines.Add(string.Format(t("readmeGameBuildTimeLabel"), buildTime));
        }

        if (includeGeneratedAt && lines.Count > 0)
        {
            lines.Add(string.Format(t("readmeGeneratedLocalTimeLabel"), GeneratedXmlFootprint.FormatGeneratedLocalTime(DateTimeOffset.Now)));
        }

        return lines;
    }

    private static void InsertBlockBefore(List<string> lines, string anchor, IReadOnlyList<string> block)
    {
        if (block.Count == 0)
        {
            return;
        ...
        int insertAt = lines.IndexOf(anchor);
        if (insertAt < 0)
        {
            insertAt = lines.Count;
        }
        else if (insertAt > 0 && string.IsNullOrEmpty(lines[insertAt - 1]))
        {
            insertAt--;
        }

        var insertLines = new List<string>(block) { "" };

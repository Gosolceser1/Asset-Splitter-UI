...
| Category | Types |
|----------|--------|
| Entry | `Program` (in `AssetProcessor.cs`) |
...
| Translation | `TranslationDictionaryLoader`, `TranslationRegistry`, `AssetNameRegistry` |
| Formatting | `FormattingService`, `FinalFormattingOrchestrator`, `OutputDirectoryManager`, `GuidFileIndex` |
...
| Progress | `AssetProcessorProgressReporter` — stdout progress lines (UI-parseable); also writes `fixer.txt` for legacy CLI polling when stdout is not redirected |

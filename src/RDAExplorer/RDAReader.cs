...
public sealed class RDAReader : IDisposable
{
    ...
    private ulong ReadBlock(ulong offset, ulong beginningOfDataSection, int blockIndex)
    {
        ...
        if (isMemoryResident)
        {
            ...
            {
                this.UpdateOutput($"Skipping memory-resident block ({blockInfo.FileCount} files) -- {ex.Message}");
                return blockInfo.NextBlock;
            }
        }
    ...
    private void ReportBlockFlags(bool isMemoryResident, bool isEncrypted, bool isCompressed, uint flags)
    {
        if (isMemoryResident)
        {
            this.UpdateOutput("MemoryResident");
        }

        if (isEncrypted)
        {
            this.UpdateOutput("Encrypted");
        }

        if (isCompressed)
        {
            this.UpdateOutput("Compressed");
        }

        if (flags == 0U)
        {
            this.UpdateOutput("No Flags");
        }
    }

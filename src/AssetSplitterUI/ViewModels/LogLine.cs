...
public sealed class LogLine
{
    ...
    public string ProgressMetrics =>
        TrySplitProgress(out string? metrics, out _, out _, out _, out _, out _) ? metrics : "";
    public string ProgressOperation =>
        TrySplitProgress(out _, out string? operation, out _, out _, out _, out _) ? operation : "";
    public string ProgressAssetDetail =>
        TrySplitProgress(out _, out _, out string? detail, out _, out _, out _) ? detail : "";
    public string ProgressGuid =>
        TrySplitProgress(out _, out _, out _, out _, out string? guid, out _) ? guid : "";
    public string ProgressGuidSeparator => HasProgressGuid && HasProgressAssetName ? " - " : "";
    public string ProgressAssetName =>
        TrySplitProgress(out _, out _, out _, out _, out _, out string? name) ? name : "";
    public string ProgressTemplateDetail =>
        TrySplitProgress(out _, out _, out _, out string? template, out _, out _) ? template : "";

...
public class StringResourceManager : INotifyPropertyChanged
{
    ...
    private static readonly Dictionary<string, string> LanguageMap = new()
  {
    ...
    public string GetString(string path)
    {
        ...
        string? result = TryGetString(_currentDocument, path)
    ...
    public static string GetLanguageCode(string displayName) =>
      LanguageMap.TryGetValue(displayName, out string? code) ? code : "";
    ...
    private void LoadStrings(string language)
    {
        try
        {
            ...
            string code = LanguageMap.TryGetValue(language, out string? c) ? c
              : (LanguageMap.ContainsValue(normalizedLanguage) ? normalizedLanguage : "");

            string filename = string.IsNullOrEmpty(code) ? "Strings.json" : $"Strings.{code}.json";

            string foundPath = ConfigPathResolver.Resolve(AppDomain.CurrentDomain.BaseDirectory, Path.Combine("Localization", "Languages"), filename);
    ...
    {
        string foundPath = ConfigPathResolver.Resolve(AppDomain.CurrentDomain.BaseDirectory, Path.Combine("Localization", "Languages"), "Strings.json");
        if (string.IsNullOrEmpty(foundPath))
        {

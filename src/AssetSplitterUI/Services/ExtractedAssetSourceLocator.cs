...
public static class ExtractedAssetSourceLocator
{
    ...
    public const string TextMetadataFileName = "texts_metadata.xml";

    private static readonly Dictionary<string, int> LanguagePopularityPriority = new(StringComparer.OrdinalIgnoreCase)
    {
        // Global popularity by total speakers (L1+L2). Regional variants stay near their
        // parent language, with the larger regional community first when both are present.
        ["english"] = 0,
        ["chinese"] = 10,
    ...
    public static IReadOnlyList<string> FindAvailableLanguages(string? outputPath, string? gamePath, string? selectedGameType)
    {
        ...
        return [.. OrderLanguagesByPopularity(languages)];
    }
    ...
    private static IEnumerable<string> EnumerateLanguageFiles(string searchPath)
    {
        try
        {
            return Directory.EnumerateFiles(searchPath, "texts_*.xml")
                .Where(IsGameLanguageFile)
                .ToArray();
        }
    ...
    public static bool IsGameLanguageFile(string filePath) =>
        !Path.GetFileName(filePath).Equals(TextMetadataFileName, StringComparison.OrdinalIgnoreCase);

    private static IOrderedEnumerable<string> OrderLanguagesByPopularity(IEnumerable<string> languages) =>
        languages
            .OrderBy(GetLanguagePopularityPriority)
            .ThenBy(language => language, StringComparer.OrdinalIgnoreCase);

    private static int GetLanguagePopularityPriority(string language) =>
        LanguagePopularityPriority.TryGetValue(language, out int priority)
            ? priority
            : int.MaxValue;
}

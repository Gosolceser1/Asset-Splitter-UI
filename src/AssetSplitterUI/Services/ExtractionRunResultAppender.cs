...
internal static class ExtractionRunResultAppender
{
        ...
        bool modExportOnly)
    {
        ...
        AppendResolvedOutputFolders(logStore, outputPath, gamePath, gameType, singleGuid, createAssetMods, modExportOnly);
        logStore.AppendRaw(Phase2Separator);
        logStore.AppendRaw("");
    }
        ...
        bool modExportOnly)
    {
        ...
        string annoAssetsRoot = GetResolvedAnnoAssetsPath(outputPath);
    ...
    private static string GetResolvedAnnoAssetsPath(string outputPath) =>
        ...
        string? gameBuildLabel = null)
    {
        ...
        if (!string.IsNullOrWhiteSpace(gameBuildLabel))
        {
            logStore.AppendLocalized("consoleMessages.gameBuildDetected", [gameBuildLabel]);
        }
    ...
    public static string? ValidatePaths(string gamePath, string outputPath)
    {
        ...
        if (AnnoInstallationDetector.TryCreateInstallation(gamePath) is null)
        {
            return "statusMessages.invalidGameDirectory";
        }

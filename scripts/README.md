...
**Output**: `artifacts/releases/Asset-Splitter-UI-v1.0.0-win-x64-self-contained.zip` and `artifacts/releases/Asset-Splitter-UI-v1.0.0-win-x64-framework.zip`.

---

### `build-common.ps1` — Shared helper functions

**Library file** — not run directly. Sourced by `clean-build.ps1`, `fast-build.ps1`, and `squash-history-and-push.ps1`.
...
**Flags**: `-c` (comments), `-f` (dependencies), `-t` (template folders), `-y` (legacy overwrite flag — passed by the script, silently ignored by the backend; output folders are cleared automatically).
...
**Flags**: `-c -f -t -y` (legacy, ignored) + optional `-CreateAssetMods`.
...
**Output**: `artifacts/debug-output/` (gitignored). Includes `debug-log.txt` and `debug-summary.txt`.
...
One-off audit and localization helpers live here so the public script folder stays easy to scan.

| File group | Purpose |
|---|---|
| `sync_*_localization.py` | Sync UI and console localization files after English strings change |
| `audit_*`, `verify_*`, `check_*` | Inspect GUID comment coverage and whitelist candidates |
| `_extract_*`, `_find_*` | Local RDA investigation helpers for Anno 117 text metadata |

Generated maintenance reports are written to `docs/reports/`.
...
| Script | Output Path |
|---|---|
| `build.ps1` | `artifacts/releases/Asset-Splitter-UI-v*.zip` |
| `debug-run.ps1` | `artifacts/debug-output/` (gitignored) |

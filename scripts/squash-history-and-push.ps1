...
$tempBranch = "squash-temp-" + (Get-Date -Format "yyyyMMddHHmmss")
...
try {
    ...
    if (-not $gitUserName -or -not $gitUserEmail) {
        if ($remote -match 'github\.com[:/](?<owner>[^/]+)/') {
            ...
            if (-not $gitUserEmail) {
                ...
                $gitUserEmail = "$owner@users.noreply.github.com"
            }
            Write-Status "INFO" "Using repo-local Git identity: $gitUserName <$gitUserEmail>" "DarkGreen"
        }
        else {
            Write-Host "[ERROR] Git user.name/user.email are not configured." -ForegroundColor Red
            Write-Host "  Run 'git config user.name ""Your Name""' and 'git config user.email ""you@example.com""' in this repo." -ForegroundColor Gray
            exit 1
        }
    }
    ...
    if (Test-Path $readmePath) {
        ...
        if ($lines -lt 20) {
            ...
            git clean -fd
            git checkout main
            & { $ErrorActionPreference = 'SilentlyContinue'; git branch -D $tempBranch 2>$null }
            exit 1
        }
    ...
    Write-Status "GIT" "Deleting old main, renaming $tempBranch -> main..."
    ...
    if ($pushExit -ne 0) {
        ...
        if ($pushText) { Write-Host $pushText -ForegroundColor Red }
        throw "git push --force origin main failed"
    }
